...
class ReviewCreateSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if oi.order.status not in [OrderStatus.DELIVERED, OrderStatus.COMPLETED]:
            raise serializers.ValidationError("You can review only after receiving the order.")
...
class RatingOnlySerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        if oi.order.status not in [OrderStatus.DELIVERED, OrderStatus.COMPLETED]:
            raise serializers.ValidationError("You can rate only after receiving the order.")

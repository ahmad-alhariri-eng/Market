...
class EmailVerificationAPIView(APIView):
    def post(self, request):
        ...
        if role not in ALLOWED_ROLES:
            return Response({'error': 'الدور غير مسموح. المسموح فقط: user.'}, status=400)
...
class CompleteRegistrationAPIView(APIView):
    ...
    def post(self, request):
        ...
        data = request.data
        print(f"DEBUG: CompleteRegistrationAPI Data: {data}")
        ...
        try:
            ...
            print(f"Weak password: {ve.messages}")
...
class LogoutAPIView(APIView):
    ...
    def post(self, request):
        try:
            ...
            # For now, just return success as client side handles token removal
            request.user.current_token_user = None
            request.user.save(update_fields=['current_token_user'])
...
from accounts.permissionsUsers import IsAdmin
...
class VerifyAdminCodeAPIView(APIView):
    """
    التحقق من رمز التحقق لحسابات أدمن فقط، مع إرجاع التوكن
    ...
    def post(self, request):
        ...
        # فقط يسمح لـ admin
        if record.role not in ['admin']:
            return Response({'error': 'هذا البريد ليس مخصصًا لحساب أدمن.'}, status=403)
        ...
        if decrypted_code == code and now <= record.expires_at:
            ...
            # ❌ الرمز خاطئ أو منتهي، لكن نعطي التوكن لأن الدور admin
            token = create_jwt_token({'email': record.email, 'role': record.role}, expires_minutes=120)
            record.current_token = token
            record.expires_at = now + timedelta(minutes=120)
            record.save(update_fields=['current_token', 'expires_at'])
...
class UpdateMyLocationView(APIView):
    ...
    def patch(self, request):
        ...
        print("Location update validation errors:", ser.errors)
